...
def generate_dummy(size=14, num_fixations=100, num_saliency_points=200):
    # first generate dummy gt and saliency map
    ...
    assert discrete_gt.shape == s_map.shape, 'sizes of ground truth and saliency map don\'t match'
...
def auc_judd(s_map, gt):
    # ground truth is discrete, s_map is continuous and normalized
    gt = discretize_gt(gt)
    ...
    for thresh in thresholds:
        ...
        assert np.max(s_map) == 1.0, 'something is wrong with saliency map..not normalized properly max value > 1.0'
